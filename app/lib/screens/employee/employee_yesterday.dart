...
class EmployeeYesterday extends StatefulWidget {
  const EmployeeYesterday({Key? key}) : super(key: key);

  @override
  State<EmployeeYesterday> createState() => _EmployeeYesterdayState();
}

class _EmployeeYesterdayState extends State<EmployeeYesterday> {
  ...
  _fetchData() async {
    ...
    Response response = await dio.get(
      ConstantVariables.yesterdayTask+"/$username",
      options: Options(
          headers: {
    ...
    int totalLength = (todayData['work'].length + todayData['meeting'].length + todayData['break'].length);
    if(totalLength!=0){
      workPercent = ((todayData['work'].length/totalLength)*100);
      meetingPercent = ((todayData['meeting'].length/totalLength)*100);
      breakPercent = ((todayData['break'].length/totalLength)*100);
    }
